...
if ($temporalCheck) {
    ...
    Write-Host "  OK $temporalVersion" -ForegroundColor Green
} else {
    Write-Host "  FAIL Temporal CLI not found" -ForegroundColor Red
...
if (Test-Path "venv\Scripts\python.exe") {
    ...
    if ($temporalioInstalled -eq "OK") {
        Write-Host "  OK temporalio package installed" -ForegroundColor Green
    } else {
        Write-Host "  FAIL temporalio package not found" -ForegroundColor Red
        Write-Host "    Run: pip install -r requirements.txt" -ForegroundColor Gray
        $allGood = $false
    }
} else {
    Write-Host "  WARN Skipping (venv not found)" -ForegroundColor Yellow
}
...
if ($allGood) {
    Write-Host "  SUCCESS All checks passed! Ready to run demo." -ForegroundColor Green
    ...
    Write-Host "  WARNING Setup incomplete. Fix issues above." -ForegroundColor Yellow
}
Write-Host "================================================" -ForegroundColor Cyan
